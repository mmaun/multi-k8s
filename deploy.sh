...
kubectl apply -f k8s
kubectl set image deployment/server-deployment server=maunconsultancy/multi-server:$SHA
kubectl set image deployment/client-deployment client=maunconsultancy/multi-client:$SHA
kubectl set image deployment/worker-deployment worker=maunconsultancy/multi-worker:$SHA
